...
void main() {
	...
	float NdotU = dot(normal,normalize(upPosition));
	...
	color.rgb = pow(color.rgb,vec3(2.2))*(vec3(1.0,0.3,0.08)*torch_lightmap*0.6 + vec3(0.003,0.003,0.004)*sqrt(0.49*NdotU+0.51)*0.45 + vec3(0.004,0.00032,0.00004)*sqrt(0.49*-NdotU+0.51)*0.33);
