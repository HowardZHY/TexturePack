...
void main() {
	...
	avgAmbient =(sun_ambient*sunVisibility + moon_ambient*moonVisibility)*eyebright*eyebright*(0.05+tr*0.15)*4.7+0.0006;
	avgAmbient2 = (sun_ambient*sunVisibility + moon_ambient*moonVisibility)*(0.05+tr*0.15)*4.7+0.0002;
	...
	eyeAdapt = log(clamp(luma(avgAmbient),0.007,80.0))/log(2.0)*0.35;
	eyeAdapt = 1.0/pow(2.5,eyeAdapt)*2.05;
	avgAmbient /= sqrt(3.0);
...
float mcosS = max(cosS,0.0);				
	...
	cloudColor = sunlight*sunVisibility*(1.0-rainStrength*0.95)*length(avgAmbient2) + avgAmbient2*0.7*(1.0-rainStrength*0.5);
}
