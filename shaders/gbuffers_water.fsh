...
//////////////////////////////ADJUSTABLE VARIABLES
	...
	#define MIX_TEX	0.7	
	#define BUMPMULT 0.015
	
vec4 watercolor = vec4(0.0,0.015,0.096,0.98); 	//water color and opacity (r,g,b,opacity)
//vec4 watercolor = vec4(0.0,0.05,0.2,0.98);
...
float factor = 3.0;
float amplitude = 0.2;
//float speed = 3.0*(1.0-rainStrength*3.3);
float speed = 3.0;
float size = 0.3;

float px = posxz.x/23.0 + 250.0;
float py = posxz.z/22.0  + 250.0;
...
factor = 1.0;
px = -posxz.x/13.0 + 250.0;
py = -posxz.z/30.0 - 250.0;
...
void main() {	
	...
	if (iswater > 0.9) {
		...
		float bumpmult = BUMPMULT*(1.0-rainStrength*12);	
